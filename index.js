...
function addStickerMetadata(webpBuffer, packName = "", stickerName = "") {
  try {
    ...
    // 22-byte TIFF Exif header for WhatsApp metadata
    const exifHeader = Buffer.from([
      0x49, 0x49, 0x2A, 0x00, // TIFF Little-Endian header ("II*\0")
      0x08, 0x00, 0x00, 0x00, // Offset to 0th IFD (8)
      0x01, 0x00,             // Number of fields in 0th IFD (1)
      0x41, 0x57,             // Tag: 0x5741 (22337 - WhatsApp metadata tag)
      0x07, 0x00,             // Type: UNDEFINED (7)
      0x00, 0x00, 0x00, 0x00, // Value count (placeholder for JSON byte length)
      0x16, 0x00, 0x00, 0x00, // Value/Offset (offset 22 = 0x16)
      0x00, 0x00, 0x00, 0x00, // Offset to next IFD (0)
    ]);
    exifHeader.writeUInt32LE(metaBuf.length, 14);

    const exifPayload = Buffer.concat([exifHeader, metaBuf]);
    // RIFF chunks must be even-length; pad with a null byte if needed
    const metaPad = exifPayload.length % 2 !== 0 ? Buffer.alloc(1, 0) : Buffer.alloc(0);
    const exifSizeBuf = Buffer.alloc(4);
    exifSizeBuf.writeUInt32LE(exifPayload.length, 0); // Exact payload length without padding
    const newExifChunk = Buffer.concat([Buffer.from("EXIF"), exifSizeBuf, exifPayload, metaPad]);
    ...
    if (isExtended) {
      ...
      // Strip any stray EXIF from the simple chunks
...
/**
 * Fallback converter for video/animation media when sharp cannot directly decode.
 * Uses ffmpeg if available, otherwise Python with OpenCV and Pillow.
 */
async function convertVideoOrGifToWebp(buffer) {
  const tempInput = path.join(tempDir, `temp_input_${Date.now()}`);
  const tempOutput = path.join(tempDir, `temp_output_${Date.now()}.webp`);
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      ff.on("close", (code) => {
        if (code === 0 && fs.existsSync(tempOutput)) resolve();
        else reject(new Error(`ffmpeg exited with code ${code}`));
      });
      ff.on("error", (err) => reject(err));
    });
  } catch (err) {
    // Fallback: Python script using cv2 & Pillow
    const pyScript = `
import sys, os
import cv2
...
if frames:
    frames[0].save(output_file, save_all=True, append_images=frames[1:], duration=frame_duration, loop=0, format="WEBP")
`;
    const tempPy = path.join(tempDir, `temp_convert_${Date.now()}.py`);
    fs.writeFileSync(tempPy, pyScript);
    await new Promise((resolve, reject) => {
      const py = spawn("python", [tempPy, tempInput, tempOutput]);
      py.on("close", (code) => {
...
/**
 * Converts a GIF or video buffer into an animated 512x512 WebP sticker.
 */
async function gifToSticker(gifBuffer, packName = "", stickerName = "") {
  ...
  try {
    ...
    console.log("Sharp animated conversion failed, trying fallback...", err.message);
    webpBuffer = await convertVideoOrGifToWebp(gifBuffer);
  }
...
function cleanJidNumber(jid) {
  ...
  const normalized = jidNormalizedUser(cleaned.includes("@") ? cleaned : `${cleaned}@s.whatsapp.net`);
...
async function startBot() {
  ...
  // Tracks pending whitelist removal sessions: senderJid -> { chatJid, whitelist, timestamp }
  const pendingWhitelistSessions = new Map();
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Check if sender has a pending whitelist session and sent a plain number
      if (/^\d+$/.test(text.trim()) && pendingWhitelistSessions.has(senderJidForSession)) {
        const session = pendingWhitelistSessions.get(senderJidForSession);
        if (Date.now() - session.timestamp > 5 * 60 * 1000) {
          pendingWhitelistSessions.delete(senderJidForSession);
        } else if (session.chatJid === chatJid) {
          const index = parseInt(text.trim(), 10);
          const { whitelist } = session;

          if (index > 0 && index <= whitelist.length) {
            const targetPhone = whitelist[index - 1];
            const badFilterStore = loadBadFilter();
            if (badFilterStore[chatJid] && Array.isArray(badFilterStore[chatJid].whitelist)) {
            ...
            pendingWhitelistSessions.delete(senderJidForSession);
            await sock.sendMessage(chatJid, {
              text: `✅ Removed *${targetPhone}* from the group whitelist.`,
            }, { quoted: msg });
          } else {
            await sock.sendMessage(chatJid, {
              text: `❌ Invalid choice. Please send a valid number from 1 to ${whitelist.length}.`,
            }, { quoted: msg });
          }
          continue;
        }
      ...
      // Check if sender has a pending badlist session and sent a plain number
      if (/^\d+$/.test(text.trim()) && pendingBadlistSessions.has(senderJidForSession)) {
        const session = pendingBadlistSessions.get(senderJidForSession);
        if (Date.now() - session.timestamp > 5 * 60 * 1000) {
          pendingBadlistSessions.delete(senderJidForSession);
        } else if (session.chatJid === chatJid) {
          const index = parseInt(text.trim(), 10);
          const { words } = session;
          ...
          if (index > 0 && index <= words.length) {
            ...
            const badFilterStore = loadBadFilter();
            if (badFilterStore[chatJid] && Array.isArray(badFilterStore[chatJid].words)) {
              badFilterStore[chatJid].words = badFilterStore[chatJid].words.filter(
            ...
            pendingBadlistSessions.delete(senderJidForSession);
            await sock.sendMessage(chatJid, {
              text: `✅ Removed *${targetWord}* from the bad word list.`,
            }, { quoted: msg });
          } else {
            await sock.sendMessage(chatJid, {
      ...
      // --- COMMAND: !ss (sticker creator / renamer / animated sticker) ---
      // Trigger: image/gif/video sent with "!ss [name]" caption
      //        : text "!ss [name]" replying to an image, gif, video, or sticker
      {
        const quotedMsg =
          msg.message?.extendedTextMessage?.contextInfo?.quotedMessage ||
          msg.message?.imageMessage?.contextInfo?.quotedMessage ||
        ...
        const quotedStickerMsg = quotedMsg?.stickerMessage;
        const quotedImageMsg = quotedMsg?.imageMessage;
        const quotedVideoMsg = quotedMsg?.videoMessage;

        const fullText =
          text ||
          directImageMsg?.caption ||
          directVideoMsg?.caption ||
          "";
        ...
        if (isSSCommand) {
          ...
          // ── Case 1: Replying to an existing sticker → rename it ──
          if (quotedStickerMsg) {
            await sock.sendMessage(chatJid, { react: { text: "⏳", key: msg.key } });
            try {
          ...
          // ── Case 2: GIF / Video (sent directly or tagged/quoted) → animated sticker ──
          const isGifOrVideo =
            directVideoMsg ||
            quotedVideoMsg ||
            (directImageMsg && directImageMsg.mimetype === "image/gif") ||
            (quotedImageMsg && quotedImageMsg.mimetype === "image/gif");

          if (isGifOrVideo) {
            await sock.sendMessage(chatJid, { react: { text: "⏳", key: msg.key } });
            try {
              const targetVideoMsg = directVideoMsg || quotedVideoMsg;
              const targetGifImageMsg = directImageMsg || quotedImageMsg;

              let mediaBuffer;
              if (targetVideoMsg) {
                mediaBuffer = await downloadMediaMessage(targetVideoMsg, "video");
              } else {
                mediaBuffer = await downloadMediaMessage(targetGifImageMsg, "image");
              }

              const stickerBuffer = await gifToSticker(mediaBuffer, stickerName, stickerName);
              await sock.sendMessage(chatJid, {
                sticker: stickerBuffer,
              }, { quoted: msg });
              await sock.sendMessage(chatJid, { react: { text: "✅", key: msg.key } });
              console.log(`🎨 Animated sticker created${stickerName ? ` with name "${stickerName}"` : ""} for ${chatJid}`);
            } catch (err) {
              console.error("Error creating animated sticker:", err);
              await sock.sendMessage(chatJid, {
                text: "❌ Failed to create animated sticker. Make sure you tagged a valid GIF or video.",
              }, { quoted: msg });
              await sock.sendMessage(chatJid, { react: { text: "❌", key: msg.key } });
            }
            continue;
          ...
          // ── Case 3: Static image (sent directly or tagged/quoted) → static sticker ──
          const targetImageMsg = directImageMsg || quotedImageMsg;
          if (targetImageMsg) {
            await sock.sendMessage(chatJid, { react: { text: "⏳", key: msg.key } });
            try {
              const imgBuffer = await downloadMediaMessage(targetImageMsg, "image");
              ...
              await sock.sendMessage(chatJid, {
                text: "❌ Failed to create sticker. Make sure you sent or tagged a valid image.",
              }, { quoted: msg });
              await sock.sendMessage(chatJid, { react: { text: "❌", key: msg.key } });
            }
            continue;
          ...
          // ── Case 4: !ss sent alone without media or quoted message ──
          if (isSSCommand) {
            await sock.sendMessage(chatJid, {
              text: "❌ *How to use !ss:*\n\n• *Create sticker:* Send an image or reply to an image with `!ss [name]`\n• *Create animated sticker:* Send a GIF/video or reply to a GIF/video with `!ss [name]`\n• *Rename sticker:* Reply to an existing sticker with `!ss <new name>` (or `!ss` to clear name)",
            }, { quoted: msg });
            continue;
          }
        }
